...
" config. {{{1
if !exists('g:ref_fish_cmd')  " {{{2
  " '-Tutf8' option is workaround for multi-byte environment.
  " The option might be unnecessary someday.
  let g:ref_fish_cmd = executable('man') ? 'man -Tutf8' : ''
endif

if !exists('g:ref_fish_lang')  " {{{2
  let g:ref_fish_lang = ''
endif


let s:source = {'name': 'fish'}  " {{{1
...
function! s:source.get_body(query)
  ...
  let cmd = ref#to_list(self.option('cmd')) + q
  ...
  try
    ...
    let res = ref#system(['fish', '-c', join(cmd)])
  finally
    if exists('lang')
      let $LANG = lang
    endif
...
function! s:source.option(opt)
  if a:opt ==# 'manpath'
    return exists('g:ref_fish_manpath') ? g:ref_fish_manpath :
    \	            $MANPATH !=# '' ? $MANPATH :
    \	            ref#system('manpath').stdout
  endif
  return g:ref_fish_{a:opt}
endfunction
...
function! ref#fish#define()
  return copy(s:source)
endfunction

call ref#register_detection('fish', 'fish')
